			...
			<table border="0" style="width:100%;">
				...
				<tr>
					<td style="height:25px; text-align:right;">
						<form name="search" method="get" action="/gyu/board/boardT.jsp">
...
<%@ include file="../include/pagD.jsp" %>
					<tr>
						...
						if( startBlock >= 10 )
						{
							%><a href="/gyu/board/boardT.jsp?kind=<%= kind %>&page=<%= startBlock - 1 %>&key=<%= key %>">이전 페이지</a> | <%
						}		
						...
						for(int pageno = startBlock; pageno <= endBlock; pageno++)
						{
							if(curpage == pageno)
							{
								...
								%><a href="/gyu/board/boardT.jsp?kind=<%= kind %>&page=<%= pageno %>&key=<%= key %>" style="color:red"><%= pageno %></a> | <%
							}else
							{
								%><a href="/gyu/board/boardT.jsp?kind=<%= kind %>&page=<%= pageno %>&key=<%= key %>"><%= pageno %></a> | <%
							}
						}	
						//(12) 다음 블럭을 출력한다.
						if( endBlock < maxpage)
						{
							%><a href="/gyu/board/boardT.jsp?kind=<%= kind %>&page=<%= endBlock + 1 %>&key=<%= key %>">다음 페이지</a><%
						}		
						%>
					</td>
				</tr>												
